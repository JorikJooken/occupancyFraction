...
SCHAR get_splitlevel(SCHAR k,SCHAR t)
{
 if(t==3)
   {
    switch(k)
...
#else
 fprintf(stderr,"splitlevel must be given with option -split\n");
#endif
 exit(0);
}
...
SCHAR default_splitlevel(SCHAR n,SCHAR k,SCHAR t,char bipartite)
{
 ...
 if (bipartite) ret += 4;     /* Added by BDM */
 if(ret+3<n)
    return(ret);
...
#else
 fprintf(stderr,"too few nodes for -m option\n");
#endif
