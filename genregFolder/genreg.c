...
/* Modified for bipartite option by Gunnar Brinkmann (July 2001) 
   -- handle with care -- never trust a program modified by someone 
  else than the author. 

   modified parts are marked: gb-mod-start, gb-mod-end. 

   Modified to add graph6 output option by Brendan McKay (August 2001).

   modified parts are marked: g6-mod-start, g6-mod-end. 
*/
...
static FILE *g6file;
static SCHAR g6all;
static SCHAR g6stdout;
static ULONG tog6;
static SCHAR nooutput;
/*********************g6-mod-end*********************/

/*********************gb-mod-start*********************/

char bipartite=0;      /* is the option bipartite used ? */
char colour[MAXN]={0}; /* the colours 1,2 of the vertices (0 means uncoloured) */
int anzcolour[3]={0};  /* anzcolour[i] the numbers of vertices with colour i \in {1,2} */
int biplimit=0;        /* the number of vertices with one colour = n/2 */
...
#else
 fprintf(stderr,"Fatal error: memory full\n");
#endif
 exit(0);
}

static void nachblist(void)
{
 SCHAR i,j;
 for(i=1;i<=n;i++)
    {
...
static void
writeg6(void)
/* graph6 output, added by Brendan McKay */
{
    unsigned char g6[20+MAXN*(MAXN-1)/12];
    unsigned char *pout;
    int nlen,bodylen,i,j,org,c;
    ...
    j = nlen + bodylen + 1;
    if (fwrite(g6,sizeof(unsigned char),(size_t)j,g6file) != j)
    {
#ifndef ENGLISH
        fprintf(stderr,">E %s: fwrite() versagt\n","genreg");
#else
...
static SCHAR girthcheck2(SCHAR mx,SCHAR my,SCHAR tw)
{
...
static SCHAR ongirth0(SCHAR v,SCHAR tw)
{
...
static SCHAR ongirth1(SCHAR v,SCHAR tw)
{
 SCHAR *status,last,a,h,i,j=0,x,y;
 status=zbk[0];
 for(i=1;i<=n;i++)status[i]=0;
...
static SCHAR knoten_als_eins(SCHAR v,SCHAR tw)
{
 SCHAR i,j,x;
 if(tw==3)
   {
...
static void sprungindex(SCHAR i,SCHAR j)
{
 SCHAR r,s,x,y,z,e;
 i1=1;j1=j;
 for(r=1;r<=i;r++)
...
static void transpinv(SCHAR *mperm)
{
 SCHAR i,x,re,li;
 i=2*mperm[0];
 while(i>0)
...
static SCHAR maxinblock(SCHAR *zeile,SCHAR *mperm,SCHAR e,SCHAR li,SCHAR re)
{
 SCHAR i,x;
 i=2*mperm[0];
 while(e>=0&&li<=re)
...
static SCHAR maxinzeile(SCHAR tz)
{
...
static SCHAR maxrekneu(SCHAR tz)
{
 SCHAR i,x,e;
 SCHAR erg;
#ifdef STAB
...
static SCHAR maxstartneu(SCHAR tw)
{
 SCHAR i,j,e;
 SCHAR erg;
#ifdef STAB
...
static SCHAR maxinzeile1(SCHAR tz)
{
 SCHAR e,li,re,erg=0;
 SCHAR b,*zeile,*block,*eintr,*mperm;
 zeile=g[kmn[tz]];
...
static SCHAR maxrekneu1(SCHAR tz)
{
 SCHAR i,x,e;
 SCHAR erg;
 if(tz==n-1)
...
static SCHAR maxstartneu1(SCHAR vm)
{
 SCHAR i,j,e;
 SCHAR erg;
 for(i=vm;i>1;i--)
	  ...
	  if(--toprint==0)
	    {
	     fclose(autfile);
#ifndef ENGLISH
	     fprintf(ergfile,"          %llu Graphen erzeugt\r",anz);
#else
	     fprintf(ergfile,"          %llu Graphs produced\r",anz);
#endif
	     fflush(ergfile);
...
/*********************gb-mod-start*********************/
	  if (!bipartite || (colour[mx]!=colour[my])) /* != works also correctly if
                                                         mx==vm and therefore mx uncoloured */
/*********************gb-mod-end*********************/
	    { 
	   g[mx][my]=g[my][mx]=1;
	   l[mx][++grad[mx]]=my;
...
/*********************gb-mod-start*********************/
	  if (bipartite && (vm==my))
	    { 
	      colour[vm]=3-colour[mx]; anzcolour[colour[vm]]++; 
	    if (anzcolour[colour[vm]]<=biplimit) ordrek(mx,my,vm,tw,i);
	    }
	  else
/*********************gb-mod-end*********************/
...
/*********************gb-mod-start*********************/
	  if (bipartite && (vm==my))
	    { anzcolour[colour[vm]]--; colour[vm]=0; }
/*********************gb-mod-end*********************/
...
/*********************g6-mod-start*********************/
 tog6=_tog6; g6all=_g6all; g6file=_g6file;
 nooutput=_nooutput;
/*********************g6-mod-end*********************/
 dez=1;
 anz=(*_anz);
...
/*********************gb-mod-start*********************/
{ int gbi,gbj,gbcolour;
if (bipartite)
  { colour[1]=1; anzcolour[1]=1;
    for(gbi=1;gbi<=i;gbi++)
      { gbcolour=3-colour[gbi];
      for(gbj=1;gbj<=grad[gbi];gbj++)
	if (l[gbi][gbj]>gbi) /* every vertex>1 hast just one smaller ancestor
				at this moment, so there is no need to check
				whether it is coloured already */
	  { colour[l[gbi][gbj]]=gbcolour;
	    anzcolour[gbcolour]++;
	  }
      }
  }
